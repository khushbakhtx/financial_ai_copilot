...
import { useFinancialCopilot } from "@/app/hooks/useFinancialCopilot";
...
export const ChatInterface = React.memo<ChatInterfaceProps>(({ assistant }) => {
  ...
  const isActuallyRunning = isLoading && !interrupt;
  const submitDisabled = isActuallyRunning || !assistant;

  const handleSubmit = useCallback(
    async (e?: FormEvent) => {
      if (e) e.preventDefault();
      const messageText = input.trim();
      if (!messageText || isActuallyRunning || submitDisabled) return;
      sendMessage(messageText);
      setInput("");
    },
    [input, isActuallyRunning, sendMessage, submitDisabled]
  );
        ...
        >
              ...
              {!metaOpen && (
                <>
                  {(() => {
                    ...
                    return (
                      <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto] items-center">
                        {tasksTrigger}
                        {filesTrigger}
                        {hasPipeline && (
                          <button
                            type="button"
                            onClick={() => setMetaOpen("pipeline")}
                  ...
                  <div className="sticky top-0 flex items-stretch bg-sidebar text-sm">
                    ...
                    {hasPipeline && (
                      <button
                        ...
                        onClick={() => setMetaOpen((prev) => prev === "pipeline" ? null : "pipeline")}
                        aria-expanded={metaOpen === "pipeline"}
                      >
                        <GitBranch size={13} />
                        Pipeline
                      </button>
                    ...
                    {hasModels && (
                      <button
                        type="button"
                        className="inline-flex items-center gap-1.5 py-3 pr-4 first:pl-[18px] aria-expanded:font-semibold"
                        onClick={() => setMetaOpen((prev) => prev === "models" ? null : "models")}
                        aria-expanded={metaOpen === "models"}
                      >
                        <Trophy size={13} />
                        Models
                      </button>
                    ...
                    {hasCharts && (
                      <button
                        type="button"
                        className="inline-flex items-center gap-1.5 py-3 pr-4 first:pl-[18px] aria-expanded:font-semibold"
                        onClick={() => setMetaOpen((prev) => prev === "charts" ? null : "charts")}
                        aria-expanded={metaOpen === "charts"}
                      >
                        <BarChart2 size={13} />
                        Charts
                      </button>
                    ...
                    <button
                      type="button"
                      className="inline-flex items-center gap-1.5 py-3 pr-4 first:pl-[18px] aria-expanded:font-semibold"
                      onClick={() => setMetaOpen((prev) => prev === "terminal" ? null : "terminal")}
                      aria-expanded={metaOpen === "terminal"}
                    >
                      <Terminal size={13} />
                      Sandbox
                    </button>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col">
            <textarea
              ...
              placeholder={isActuallyRunning ? "Running..." : "Write your message..."}
            ...
            <div className="flex items-center justify-between gap-2 p-3">
              ...
              <Button
                type={isActuallyRunning ? "button" : "submit"}
                variant={isActuallyRunning ? "destructive" : "default"}
                onClick={isActuallyRunning ? stopStream : handleSubmit}
                disabled={!isActuallyRunning && (submitDisabled || !input.trim())}
              >
                {isActuallyRunning ? (
                  <>
                    <Square size={14} />
